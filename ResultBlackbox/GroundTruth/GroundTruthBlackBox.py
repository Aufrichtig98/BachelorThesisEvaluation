...
def feature_permutation(feature_list: List[str], legal:List[List[str]]) -> List[List[str]]:
    """
    Calculates all feature permutation

    Args:
        feature_list (List[str]): All features
        legal (List[List[str]]): Legal features

    Returns:
        List[List[str]]: List with list of features, list with single elments are single feature else they interact
    """
...
def feature_string_list(str_configuration: List[str], Shared=False):
    """
    Transforms list with active features as strings and maps them to a list of ints where each int represents a feature

    Args:
        str_configuration (List[str]): Active features as string
        Shared (bool, optional): If Shared Featura Variables is the current system. Defaults to False.
...
def feature_black_box(filename, Shared=False):
    """
    Reads filename, and sets the active features in a list of int where each position represents a feature

    Args:
        filename (_type_): Name of the configuration
        Shared (bool, optional): If system is shared feature variable . Defaults to False.

    Returns:
        _type_: List, where each position represents a feature, if active 1 else 0
    """
    if (Shared):
        result_list = [0] * 5
    else:
...
class multible_linear_reagression:
    ...
    def config_to_matrix(self, config:tuple, Shared=False):
        ...
        Sets active features, also base

        Args:
            config (tuple): active features
            Shared (bool, optional): _description_. Defaults to False.
    ...
    def __init__(self, feature_config_to_time:dict, path:Path):
        ...
        #Driver code to build the multiple linear regression model 
        #Builds the performance influence model for the black-box ground truth systems  and writes it as latex table into a file
